...
const blogFields = `title,subtitle,"slug": slug.current, download, downloadLink ,coverImage, 'file': file.asset->url ,date, tags, 'author': author->{name ,'alt': avatar.alt,bio,'avatar': avatar.asset->url}`;
...
export async function getOnePopularBlogs() {
  const results = await client.fetch(
    `*[_type == "blog"] {${blogFields},content} | order(popularity desc)[0] `
  );
  return results;
}
