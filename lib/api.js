...
export async function getBlogsByCategory(category) {
  const results = await client.fetch(
    `*[_type == 'blog' && ${category} in categories[].category->title] {${blogFields} ,content[]{...,"asset": asset->, markDefs[]{
  		...,
      _type == "internalLink" => {
        "slug": @.reference->slug,       
      }
...
export async function getCategories() {
  const results = await client.fetch(
    `*[_type == "category"]{title, description,'icon': { 'image': icon.asset->url, 'alt': icon.alt}}`
  );
  return results;
}
