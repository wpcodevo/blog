...
const blogFields = ` _id,title,subtitle,"slug": slug.current, download, downloadLink ,coverImage, 'file': file.asset->url ,date, tags, 'author': author->{name ,'alt': avatar.alt,bio,'avatar': avatar.asset->url}`;
...
export async function getBlogBySlug(slug, preview) {
  ...
  const result = await currentClient
    .fetch(
      ...
      'comments': *[_type == "comment" && blog._ref == ^._id]{_id, name, email, comment, _createdAt},content[]{...,"asset": asset->, markDefs[]{
  		...,
      _type == "internalLink" => {
        "slug": @.reference->slug,       
      }
...
export async function getNewsBlogs() {
  const results = await client.fetch(
    `*[_type == 'blog' && 'news' in categories[]->slug.current] {${blogFields} ,content[]{...,"asset": asset->, markDefs[]{
  		...,
      _type == "internalLink" => {
        "slug": @.reference->slug,       
      }
...
export async function getTwoPopularBlogs() {
  const results = await client.fetch(
    `*[_type == "blog"] {${blogFields},content} | order(popularity desc)[0..1] `
  );
  return results;
}
