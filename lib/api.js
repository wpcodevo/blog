...
const blogFields = `title,subtitle,"slug": slug.current, coverImage, 'file': file.asset->url ,date, tags, 'author': author->{name ,'avatar': avatar.asset->url}`;
...
//   const results = await client.fetch(`*[_type == "blog"] | order(date desc)`);
//   return results;
// }
export async function getBlogs() {
  const results = await client.fetch(
